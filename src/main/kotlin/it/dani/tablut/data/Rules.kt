...
class Rules {
    fun genFeasibleMoves(board: TablutBoard): List<Move> {
        val result : MutableList<Move> = ArrayList()

        val boardData = BoardData.extractData(board)
        BoardData.getTurnPositions(boardData, board.turn).forEach { position ->
            result.addAll(this.genFeasibleMoves(board,position))
        }

        return result
    }

    private fun genFeasibleMoves(board: TablutBoard, position: Position): List<Move> {
        val result: MutableList<Move> = LinkedList()

        result += this.genFeasibleHorizontalMoves(board, position)
        result += this.genFeasibleVerticalMoves(board, position)

        return result
    }

    private fun genFeasibleHorizontalMoves(board: TablutBoard, position: Position): List<Move> {
        val result: MutableList<Move> = LinkedList()
        var count = 1
        var flagStop = false
    ...
    private fun genFeasibleVerticalMoves(board: TablutBoard, position: Position): List<Move> {
        val result: MutableList<Move> = LinkedList()
        var count = 1
        var flagStop = false
